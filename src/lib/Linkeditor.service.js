...
export class LinkeditorService {
	...
	static registerFileActions() {
		// Register actions for each supported mimetype
		...
		if (directDownload && directDownload.length > 0) {
			...
			// Public download page, single file - support .url, .webloc, and .mtd
			if (extension === "url" || extension === "webloc" || extension === "mtd") {
